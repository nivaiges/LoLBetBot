...
export const data = new SlashCommandBuilder()
  ...
  .addBooleanOption(opt =>
    opt.setName('smurf')
      .setDescription('Mark this account as a smurf (defaults to main)')
      .setRequired(false)
  );
...
export async function execute(interaction) {
  ...
  const isSmurf = interaction.options.getBoolean('smurf') ?? false;
  ...
  addTrackedPlayer(guildId, riotId, account.puuid, region, isSmurf);
  const label = isSmurf ? ' _(smurf)_' : '';
  return interaction.editReply(`✅ Now tracking **${riotId}**${label} for match betting.`);
}
